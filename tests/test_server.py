...
def server():
    ...
    with patch("mcp_server_rabbitmq.server.FastMCP"):
...
class TestRabbitMQTools:
    ...
    def test_list_queues(self, server):
        ...
        with patch("mcp_server_rabbitmq.server.RabbitMQAdmin") as mock_admin:
            with patch("mcp_server_rabbitmq.server.handle_list_queues") as mock_handle_list_queues:
                ...
                mock_admin.assert_called_once_with("localhost", 15672, "guest", "guest", False)
    ...
    def test_list_exchanges(self, server):
        ...
        with patch("mcp_server_rabbitmq.server.RabbitMQAdmin") as mock_admin:
            with patch(
                "mcp_server_rabbitmq.server.handle_list_exchanges"
            ) as mock_handle_list_exchanges:
                mock_admin.return_value = MagicMock()
    ...
    def test_get_queue_info(self, server):
        ...
        with patch("mcp_server_rabbitmq.server.RabbitMQAdmin") as mock_admin:
            with patch(
                "mcp_server_rabbitmq.server.handle_get_queue_info"
            ) as mock_handle_get_queue_info:
                with patch("mcp_server_rabbitmq.server.validate_rabbitmq_name"):
                    mock_admin.return_value = MagicMock()
                    mock_handle_get_queue_info.return_value = {
                        "name": "test_queue",
                        "messages": 10,
                    }
                    ...
                    assert (
                        get_queue_info_func is not None
                    ), "Could not find get_queue_info function"

                    # Call the function directly
                    result = get_queue_info_func("test_queue")

                    mock_admin.assert_called_once_with("localhost", 15672, "guest", "guest", False)
                    mock_handle_get_queue_info.assert_called_once_with(
                        mock_admin.return_value, "test_queue", "/"
                    )
    ...
    def test_delete_queue(self, server):
        ...
        with patch("mcp_server_rabbitmq.server.RabbitMQAdmin") as mock_admin:
            with patch(
                "mcp_server_rabbitmq.server.handle_delete_queue"
            ) as mock_handle_delete_queue:
                with patch("mcp_server_rabbitmq.server.validate_rabbitmq_name"):
                    mock_admin.return_value = MagicMock()
                    ...
                    mock_admin.assert_called_once_with("localhost", 15672, "guest", "guest", False)
                    mock_handle_delete_queue.assert_called_once_with(
                        mock_admin.return_value, "test_queue", "/"
                    )
        ...
        with patch("mcp_server_rabbitmq.server.RabbitMQAdmin") as mock_admin:
            with patch("mcp_server_rabbitmq.server.handle_purge_queue") as mock_handle_purge_queue:
                with patch("mcp_server_rabbitmq.server.validate_rabbitmq_name"):
                    ...
                    mock_admin.assert_called_once_with("localhost", 15672, "guest", "guest", False)
                    mock_handle_purge_queue.assert_called_once_with(
                        mock_admin.return_value, "test_queue", "/"
                    )
    ...
    def test_delete_exchange(self, server):
        ...
        with patch("mcp_server_rabbitmq.server.RabbitMQAdmin") as mock_admin:
            with patch(
                "mcp_server_rabbitmq.server.handle_delete_exchange"
            ) as mock_handle_delete_exchange:
                with patch("mcp_server_rabbitmq.server.validate_rabbitmq_name"):
                    mock_admin.return_value = MagicMock()
                    ...
                    assert (
                        delete_exchange_func is not None
                    ), "Could not find delete_exchange function"

                    # Call the function directly
                    result = delete_exchange_func("test_exchange")

                    mock_admin.assert_called_once_with("localhost", 15672, "guest", "guest", False)
                    mock_handle_delete_exchange.assert_called_once_with(
                        mock_admin.return_value, "test_exchange", "/"
                    )
                    assert "successfully deleted" in result
    ...
    def test_get_exchange_info(self, server):
        ...
        with patch("mcp_server_rabbitmq.server.RabbitMQAdmin") as mock_admin:
            with patch(
                "mcp_server_rabbitmq.server.handle_get_exchange_info"
            ) as mock_handle_get_exchange_info:
                with patch("mcp_server_rabbitmq.server.validate_rabbitmq_name"):
                    mock_admin.return_value = MagicMock()
                    mock_handle_get_exchange_info.return_value = {
                        "name": "test_exchange",
                        "type": "fanout",
                    }
                    ...
                    assert (
                        get_exchange_info_func is not None
                    ), "Could not find get_exchange_info function"

                    # Call the function directly
                    result = get_exchange_info_func("test_exchange")

                    mock_admin.assert_called_once_with("localhost", 15672, "guest", "guest", False)
                    mock_handle_get_exchange_info.assert_called_once_with(
                        mock_admin.return_value, "test_exchange", "/"
                    )
